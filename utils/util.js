  ...
  var promiseFun = function (resolve, reject) {
    ...
    wx.request({
      ...
      complete: function (res) {
        ...
        //异常日志统一处理 保存到服务器数据库
        if (res.data && res.data.code && res.data.code == 1004) {
          getErrorlog({
            url: url,
      ...
      wx.request({
        ...
        success: function (res) {
          ...
          if (data.access_token) {
            ...
            showModal('收收提示', '登录过期，请重新登录', '登录', '取消', function (res) {
              if (res.confirm) {
                wx.redirectTo({
          ...
          that.https(app.globalData.api + "/api/wc/GetOpenid", "GET", {
            ...
            function (data) {
              if (data.code == 1001) {
                ...
                if (islogin) { //自动登录
                  if (data.data.UserLogID != null && data.data.UserLogID != "" && data.data.usersecret != null && data.data.usersecret != "") {
...
function getCurrentCity(that, level, callback) {
  ...
  var getArea = function () {
    https("https://restapi.amap.com/v3/geocode/regeo", "GET", {
  ...
  //是否有缓存经纬度
  if (Number(that.data.handlongitude || wx.getStorageSync("longitude")) == 0 && Number(that.data.handlatitude || wx.getStorageSync("latitude"))==0 ){
    getLocation(that,function(){
      getArea();
    })
  }else{
    getArea();
 ...
 * 获取银行列表
 */
function getBankList(that, params, callback) {
  this.https(app.globalData.api + "/api/bank/get_list/" + wx.getStorageSync("userid"), "GET", params,
    function (data) {
      callback.call(this, data)
    })
}
